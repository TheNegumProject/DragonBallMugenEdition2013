                              ;Artificial Inteligencia
[Command]
name = "IA1"
command = a+b+c+z+x+y+U+D+F+B
time = 1

[Command]
name = "IA2"
command = ~D, DF, F, b,a+b+c+z+x+y+U+D+F+B
time = 1

[Command]
name = "IA3"
command = ~D, DF, F, c,a+b+c+z+x+y+U+D+F+B
time = 1

[Command]
name = "IA4"
command = ~D, DF, F, x,a+b+c+z+x+y+U+D+F+B
time = 1

[Command]
name = "IA5"
command = ~D, DF, F, y,a+b+c+z+x+y+U+D+F+B
time = 1

[Command]
name = "IA6"
command = ~D, DF, F, z,a+b+c+z+x+y+U+D+F+B
time = 1

[Command]
name = "IA7"
command = ~D, DB, B, a,a+b+c+z+x+y+U+D+F+B
time = 1

[Command]
name = "IA8"
command = ~D, DB, B, b,a+b+c+z+x+y+U+D+F+B
time = 1

[Command]
name = "IA9"
command = ~D, DB, B, c,a+b+c+z+x+y+U+D+F+B
time = 1

[Command]
name = "IA10"
command = ~D, DB, B, x,a+b+c+z+x+y+U+D+F+B
time = 1

[Command]
name = "IA11"
command = ~D, DB, B, y,a+b+c+z+x+y+U+D+F+B
time = 1

[Command]
name ="IA12"
command = ~D, DB, B, z,a+b+c+z+x+y+U+D+F+B
time = 1

[Command]
name = "IA13"
command = ~D, DB, B, c,a+b+c+z+x+y+U+D+F+B
time = 1

[Command]
name = "IA14"
command = ~D, DB, B, x,a+b+c+z+x+y+U+D+F+B
time = 1

[Command]
name = "IA15"
command = ~D, DB, B, y,a+b+c+z+x+y+U+D+F+B
time = 1

[Command]
name ="IA16"
command = ~D, DB, B, z,a+b+c+z+x+y+U+D+F+B
time = 1
;--------------------
[Command] 
name = "sf_flash"
command = ~D, DB, B, F, a
time = 30
[Command]
name = "ussj"
command = a+b
time = 1

[Command]
name = "kamehameha10"
command = ~D, DB, B, F, y
time = 30

[Command]
name = "f_flash"
command = ~D, DB, B, F, x
time = 30

[Command]
name = "hf_flash"
command = ~D, DB, B, F, c
time = 30
[Command]
name = "b_atack"
command = ~D, DB, B, F, b
time = 30
[Command]
name = "magias"
command = ~D, DB, B, F, z
time = 30
[Command]
name = "kenzien"
command = ~D, DB, B, F, y
time = 30

[Command] 
name = "meteor_atack_2"
command = ~D, DF, F, B, x
time = 30


[Command]
name = "ussj meteor_atack_2"
command = ~D, DB, B, F, z
time = 30

[Command] 
name = "m_atack"
command = ~D, DF, F, B, a
time = 30

[Command]
name = "ussj m_atack"
command = ~D, DB, B, F, c
time = 30


[Command]
name = "bakuhatsu_ha"
command = ~D, DB, B, a
time = 10

[Command]
name = "s_soco"
command = ~D, DF, F, a
time = 10


[Command]
name = "balao"
command = /F,a

[Command]
name = "balao"
command = /F,b


[Command]
name = "balao"
command = /F,c
[command]
name ="teleporte"
command = x+a



[Command]
name = "cutu_a"
command = ~D, F, y
time = 10

[Command]
name = "cut_soco"
command = ~D, B, y
time = 10

[Command]
name = "joel_a"
command = ~D, F, x
time = 10

[Command]
name = "jog_chut"
command = ~D, B, x
time = 10

[Command]
name = "fireball"
command = ~D, DF, F, c
time = 10

[Command]
name = "fireballb"
command = ~D, DB, B, c
time = 10

[Command]
name = "fireball3"
command = ~D, DF, F, z
time = 10

[Command]
name = "magic_t"
command = ~D, DB, B, z
time = 10

[Command]
name = "sequence"
command = ~D, DB, B, b
time = 10

[Command]
name = "c_e_alto"
command = ~D, DB, B, a
time = 10

[Command]
name = "c_e_alto"
command = ~D, DB, B, b

[Command]
name = "c_e_alto"
command = ~D, DB, B, c
time = 10

[Command]
name = "garlic roou"
command = D, DB, B, F, x
[Command]
name = "magiasussj"
command = D, DB, B, F, a
[Command]
name = "QCF_b"
command = ~D, DF, F, b
time = 10

[Command]
name = "QCF_c"
command = ~D, DF, F, c

[Command]
name = "QCB_b"
command = ~D, DB, B, b
time = 10

[Command]
name = "QCB_c"
command = ~D, DB, B, c

[Command]
name = "QCB_x"
command = ~D, DB, B, x

[Command]
name = "QCB_y"
command = ~D, DB, B, y

[Command]
name = "uppercut_a"
command = ~F, D, DF, a

[Command]
name = "uppercut_b"
command = ~F, D, DF, b

[Command]
name = "uppercut_c"
command = ~F, D, DF, c

[Command]
name = "uppercut_x"
command = ~F, D, DF, x

[Command]
name = "uppercut_y"
command = ~F, D, DF, y

[Command]
name = "uppercut_z"
command = ~F, D, DF, z

[Command]
name = "chargedownup_a"
command = ~60$D, U, a
time = 10

[Command]
name = "chargedownup_b"
command = ~60$D, U, b
time = 10

[Command]
name = "chargedownup_c"
command = ~60$D, U, c
time = 10

[Command]
name = "chargedownup_x"
command = ~60$D, U, x
time = 10

[Command]
name = "chargedownup_y"
command = ~60$D, U, y
time = 10

[Command]
name = "chargedownup_z"
command = ~60$D, U, z
time = 10

[Command]
name = "chargebackfwd_a"
command = ~60$B, F, a
time = 10

[Command]
name = "chargebackfwd_b"
command = ~60$B, F, b
time = 10

[Command]
name = "chargedownup_c"
command = ~60$D, U, c
time = 10

[Command]
name = "chargedownup_x"
command = ~60$D, U, x
time = 10

[Command]
name = "chargedownup_y"
command = ~60$D, U, y
time = 10

[Command]
name = "chargedownup_z"
command = ~60$D, U, z
time = 10


[Command]
name = "FF";Required (do not remove)
command = F, F
time = 10

[Command]
name = "BB";Required (do not remove)
command = B, B
time = 10

[Command]
name = "UU"
command = U, U
time = 10

[Command]
name = "DD"
command = D, D
time = 10

[Command]
name = "holda"
command = /a
time = 1

[Command]
name = "holdb"
command = /b
time = 1

[Command]
name = "recovery"
command = a+b
time = 1

[Command]
name = "ret_mag"
command = y+x
time = 1

[Command]
name = "charge"
command = /a
time = 1

[Command]
name = "charge 1"
command = /b
time = 1

[Command]
name = "fly"
command = y+b
time = 30

[Command]
name = "fly_air"
command = b+y
time = 30

[Command]
name = "fwd_a"
command = /F,a
time = 1

[Command]
name = "fwd_b"
command = /F,b
time = 1

[Command]
name = "fwd_c"
command = /F,c
time = 1

[Command]
name = "fwd_x"
command = /F,x
time = 1

[Command]
name = "fwd_y"
command = /F,y
time = 1

[Command]
name = "fwd_z"
command = /F,z
time = 1

[Command]
name = "back_a"
command = /B,a
time = 1

[Command]
name = "back_b"
command = /B,b
time = 1

[Command]
name = "back_c"
command = /B,c
time = 1

[Command]
name = "down_a"
command = /$D,a
time = 1

[Command]
name = "down_b"
command = /$D,b
time = 1

[Command]
name = "down_c"
command = /$D,c
time = 1

[Command]
name = "fwd_ab"
command = /F, a+b
time = 1

[Command]
name = "back_ab"
command = /B, a+b
time = 1

[Command]
name = "a"
command = a
time = 1

[Command]
name = "b"
command = b
time = 1

[Command]
name = "c"
command = c
time = 1

[Command]
name = "x"
command = x
time = 1

[Command]
name = "y"
command = y
time = 1

[Command]
name = "z"
command = z
time = 1

[Command]
name = "s"
command = s
time = 1

[Command]
name = "holdfwd";Required (do not remove)
command = /$F
time = 1

[Command]
name = "holdback";Required (do not remove)
command = /$B
time = 1

[Command]
name = "holdup";Required (do not remove)
command = /$U
time = 1

[Command]
name = "holddown"
command = /$D
time = 1
[Statedef -1]
[State -1]
type = varset
triggerall = var(13) != 1
trigger1 = command = "IA1"
trigger2 = command = "IA2"
trigger3 = command = "IA3"
trigger4 = command = "IA4"
trigger5 = command = "IA5"
trigger6 = command = "IA6"
trigger7 = command = "IA7"
trigger8 = command = "IA8"
trigger9 = command = "IA9"
trigger10 = command = "IA10"
trigger11 = command = "IA11"
trigger12 = command = "IA12"
trigger13 = command = "IA13"
trigger14 = command = "IA14"
trigger15 = command = "IA15"
trigger16 = command = "IA16"
v = 13
value = 1



[State -1]
type = changestate
value = 1060989
triggerall = var (5) = 1
triggerall = command ="QCF_b"
trigger1 = statetype = S
trigger1 = ctrl = 1
[State -1]
type = ChangeState
value = 1050989
triggerall = var (5) = 1
triggerall = command = "sequence"
triggerall = power >= 300
trigger1 = statetype = S
trigger2 = ctrl = 1
trigger3 = stateno = 11500
[State -1, Fast Punch]
type = ChangeState
value = 1020989
triggerall = var (5) = 1
triggerall = command = "s_soco"
triggerall = power >= 0
trigger1 = statetype = S
trigger1 = ctrl
trigger2 = stateno = 11500
trigger3 = (stateno = [10200,10299]) || (stateno = [10211,10212])
trigger3 = movecontact

[State -1]
type = ChangeState
value = 2991989
triggerall = var (5) = 1
triggerall = command = "cutu_a"
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 11500
[State -1]
type = ChangeState
value = 2992989
triggerall = var (5) = 1
triggerall = command = "joel_a"
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 11500
[State -1]
type = ChangeState
value = 360989
triggerall = var (5) = 1
triggerall = command = "cut_soco"
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 11500

[State -1]
type = ChangeState
value = 350989
triggerall = var (5) = 1
triggerall = command = "jog_chut"
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 11500

 [State -1]
type = ChangeState
value = 3200989
triggerall = var (5) = 1
triggerall = command = "magiasussj"
triggerall = power >= 1000
trigger1 = statetype != A
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 11500
 [State -1]
type = ChangeState
value = 2600989
triggerall = statetype = S && Var(5) = 1
triggerall = command = "bakuhatsu_ha"
triggerall = command != "holddown"
triggerall = numprojid(1696) = 0
trigger1 = statetype = S
trigger1 = ctrl = 1

[State -1]
type = ChangeState
value = 90099
triggerall = statetype = S && Var(5) = 1
triggerall = ctrl = 1
triggerall = p2bodydist X < 5 ;Near P2
trigger1 = command = "fwd_b";p2 stand
trigger1 = stateno != 100    ;Not running
trigger1 = p2statetype = S
trigger1 = p2movetype != H
trigger2 = command = "fwd_b";p2 crouch
trigger2 = stateno != 100    ;Not running
trigger2 = p2statetype = C
trigger2 = p2movetype != H
trigger3 = command = "back_b";p2 stand
trigger3 = p2statetype = S
trigger3 = p2movetype != H
trigger4 = command = "back_b";p2 crouch
trigger4 = p2statetype = C
trigger4 = p2movetype != H
[State -1]
type = ChangeState
value = 195989
triggerall = var (5) = 1
trigger1 = command = "s"
trigger1 = statetype = S
trigger1 = stateno != 312
trigger1 = ctrl = 1

[State -1]
type = ChangeState
value = 700989
triggerall = var (5) = 1
triggerall = command = "ret_mag"
trigger1 = statetype != A
trigger1 = ctrl = 1
trigger2 = stateno = 11500
[State -1]
type = ChangeState
value = 11510
triggerall = var (5) = 1
trigger1 = statetype != A
trigger1 = ctrl = 1
trigger1 = command = "fly"
[State -1]
type = ChangeState
value = 11500
triggerall = var (5) = 1
trigger1 = statetype = A
trigger1 = ctrl = 1
trigger1 = Pos Y < 0
trigger1 = command = "fly_air"
[State -1]
type = ChangeState
value = 7800989
triggerall = var (5) = 1
triggerall = command = "ussj meteor_atack_2"
triggerall = power >= 3000
triggerall = numproj = 0
trigger1 = ctrl = 1
trigger1 = statetype != A
[State -1]
type = ChangeState
value = 336989
triggerall = var (5) = 1
triggerall = command = "teleporte"
trigger1 = statetype != A
trigger1 = ctrl = 1
trigger2 = stateno = 1510
trigger3 = stateno = 1500
[State -1]
type = ChangeState
value = 4010989
triggerall = var (5) = 1
triggerall = command = "ussj m_atack"
triggerall = power >= 3000
triggerall = numproj = 0
trigger1 = ctrl = 1
trigger1 = statetype != A
[State -1]
type = ChangeState
value = 2500
triggerall = var (5) = 1
triggerall = command = "kamehameha10"
triggerall = numproj = 0
triggerall = power > 100
trigger1 = statetype != C
trigger1 = ctrl = 1
trigger2 = stateno = 900
[State -1, Final Flash]
type = ChangeState
value = 7000
triggerall = var(5) = 1
triggerall = command ="garlic roou"
trigger1 = statetype = S && Ctrl && power >= 1000
[State -1]
type = ChangeState
value = 1900
triggerall = command = "b_atack"
triggerall = numproj = 0
triggerall = power >= 1000
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 1500
[State -1]
type = ChangeState
value = 1600
triggerall = command = "hf_flash"
triggerall = var (5) = 0
triggerall = power >= 3000
triggerall = stateno != 1600
triggerall = numprojid(1700) = 0
trigger1 = statetype != A
trigger1 = ctrl = 1
trigger2 = stateno = 1500
trigger3 = statetype != A
trigger3 = hitdefattr = SC, NA, SA
trigger3 = movecontact = 1
[State -1]
type = ChangeState
value = 3200
triggerall = var (5) = 0
triggerall = command = "magias"
triggerall = power >= 1000
trigger1 = statetype != A
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 1500
[State -1]
type = ChangeState
triggerall = var (5) = 0
value = 1000
triggerall = command = "fireball"
triggerall = numproj = 0
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 1500
[State -1]
type = ChangeState
triggerall var (5) = 1
value = 1000989
triggerall = command = "fireball"
triggerall = numproj = 0
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 1500
[State -1]
type = ChangeState
triggerall = var (5) = 0
value = 1099
triggerall = command = "fireballb"
triggerall = numproj = 0
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 1500
[State -1]
type = ChangeState
triggerall = var (5) = 0
value = 1055
triggerall = command = "magic_t"
triggerall = numproj = 0
trigger1 = statetype != A
trigger1 = ctrl = 1
trigger2 = stateno = 1500
[State -1]
type = ChangeState
triggerall = var (5) = 0
value = 1390
triggerall = command = "fireball3"
triggerall = numproj = 0
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 1500
[State -1]
type = ChangeState
value = 1055989
triggerall var (5) = 1
triggerall = command = "magic_t"
triggerall = numproj = 0
trigger1 = statetype != A
trigger1 = ctrl = 1
trigger2 = stateno = 1500
[State -1]
type = ChangeState
triggerall var (5) = 1
value = 1099989
triggerall = command = "fireballb"
triggerall = numproj = 0
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 1500
[State -1]
type = ChangeState
value = 1390989
triggerall var (5) = 1
triggerall = command = "fireball3"
triggerall = numproj = 0
trigger1 = statetype = S
trigger1 = ctrl = 1

[State -1]
type            = ChangeState
value      = 10200
triggerall = var (5) = 1
triggerall      = (command = "x")
triggerall      = command != "holddown"
trigger1        = (statetype = S) && (ctrl)
trigger2  =  movecontact = 1
trigger3        = stateno = 11500
[State -1]
type            = ChangeState
value      = 10201
triggerall = var (5) = 1
triggerall      = (command = "y")
triggerall      = command != "holddown"
trigger1        = (statetype = S) && (ctrl)
trigger2        = (stateno = 10200) || (stateno = 10211)|| (stateno = 10210) && (time > 10) ||(stateno = 10212) || (stateno = 10301)|| (stateno = 10300)|| (stateno = 10303)
trigger2        = (movecontact)
trigger2  =  movecontact = 1
trigger3        = stateno = 11500
[State -1]
type            = ChangeState
value      = 10202
triggerall = var (5) = 1
triggerall      = (command = "z")
trigger1        = (statetype = S) && (ctrl)
trigger2        = (stateno = 10200) || (stateno = 10201)|| (stateno = 10210) && (time > 10) ||(stateno = 10211) || (stateno = 10301)|| (stateno = 10300)|| (stateno = 10303)
trigger2  =  movecontact = 1
trigger3= stateno = 11500

[State -1]
type            = ChangeState
value      = 10210
triggerall = var (5) = 1
triggerall      = (command = "a")
triggerall      = command != "holddown"
trigger1        = (statetype = S) && (ctrl)
trigger2        = (stateno = 10200)||(stateno = 10201)|| (stateno = 10201) || (stateno = 10200)|| (stateno = 300)|| (stateno = 303)
trigger2        = (movecontact)
trigger3        = stateno = 11500
[State -1]
type            = ChangeState
value      = 10211
triggerall = var (5) = 1
triggerall      = (command = "b")
triggerall      = command != "holddown"
trigger1        = (statetype = S) && (ctrl)
trigger2        = (stateno = 10210) || (stateno = 10210)|| (stateno = 10210)|| (stateno = 10210) && (time > 10)||(stateno = 302) || (stateno = 301)|| (stateno = 300)|| (stateno = 303)
trigger2        = (movecontact)
trigger3        = stateno = 11500
[State -1]
type            = ChangeState
value      = 10212
triggerall = var (5) = 1
triggerall      = (command = "c")
trigger1        = (statetype = S) && (ctrl)
trigger2        = (stateno = 10200) || (stateno = 10211)|| (stateno = 10201)|| (stateno = 10210) ||(stateno = 213) && (time > 10)||(stateno = 302) || (stateno = 301)|| (stateno = 300)|| (stateno = 303)
trigger2        = (movecontact)
trigger3= stateno = 11500
[State -1]
type = changestate
triggerall = var (5) = 0
value = 2999
triggerall = power >= 3000
trigger1 = command = "ussj"
trigger1 = statetype = S
[State -1]
type = changestate
value = 1060
triggerall = command ="QCF_b"
trigger1 = statetype = S
trigger1 = ctrl = 1
[State -1]
type = ChangeState
value = 4010
triggerall = command = "m_atack"
triggerall = power >= 3000
triggerall = numproj = 0
trigger1 = ctrl = 1
trigger1 = statetype != A
[State -1]
type = ChangeState
value = 7800
triggerall = command = "meteor_atack_2"
triggerall = power >= 2000
triggerall = numproj = 0
trigger1 = ctrl = 1
trigger1 = statetype != A
[State -1]
type = ChangeState
value = 1615
triggerall = var (5) = 0
triggerall = command = "sf_flash"
triggerall = power >= 1000 ;Level 3 Super
triggerall = stateno != 1700 
triggerall = numprojid(1700) = 0
trigger1 = statetype != A
trigger1 = ctrl = 1
trigger2 = stateno = 1500
trigger3 = statetype != A
trigger3 = hitdefattr = SC, NA, SA
trigger3 = movecontact = 1
[State -1]
type = ChangeState
value = 1700
triggerall = var (5) = 0
triggerall = command = "f_flash"
triggerall = power >= 2000 ;Level 3 Super
triggerall = stateno != 1700 
triggerall = numprojid(1700) = 0
trigger1 = statetype != A
trigger1 = ctrl = 1
trigger2 = stateno = 1500
trigger3 = statetype != A
trigger3 = hitdefattr = SC, NA, SA
trigger3 = movecontact = 1
[State -1, Fast Punch]
type = ChangeState
value = 1020
triggerall = command = "s_soco"
triggerall = power >= 0
trigger1 = statetype = S
trigger1 = ctrl
trigger2 = stateno = 1500
trigger3 = (stateno = [200,299]) || (stateno = [400,499]) 
trigger3 = movecontact
[State -1]
type = ChangeState
value = 1950
triggerall = command = "kenzien";-------
triggerall = numproj = 0
triggerall = power >= 1500 ;Level 2 Super
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 1500
trigger3 = stateno = 200        ;Stand_a
trigger4 = stateno = 300        ;Stand_fwd_a
trigger4 = movecontact = 1
trigger5 = stateno = 500        ;Crouch_fwd_a
trigger5 = time <= 2
trigger6 = stateno = 410        ;Crouch_b
trigger6 = movecontact = 1
trigger7 = stateno = 210        ;stand_b
trigger7 = movecontact = 1

[State -1]
type = ChangeState
value = 2600
triggerall = command = "bakuhatsu_ha"
triggerall = command != "holddown"
triggerall = numprojid(2650) = 0
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 200
trigger2 = movecontact = 1  
trigger3 = stateno = 210
trigger3 = movecontact = 1  
trigger4 = stateno = 220
trigger4 = movecontact = 1  
trigger5 = stateno = 230
trigger5 = movecontact = 1  
trigger6 = stateno = 240
trigger6 = movecontact = 1
trigger7 = stateno = 400
trigger7 = movecontact = 1  
trigger8 = stateno = 410
trigger8 = movecontact = 1  
trigger9 = stateno = 420
trigger9 = movecontact = 1  
trigger10 = stateno = 430
trigger10 = movecontact = 1  
[State -1]
type = ChangeState
value = 1050
triggerall = command = "sequence"
triggerall = power >= 300
trigger1 = statetype = S
trigger2 = ctrl = 1
trigger3 = stateno = 1500
trigger4 = stateno = 200
trigger4 = movecontact = 1
trigger5 = stateno = 210
trigger5 = movecontact = 1
trigger6 = stateno = 211
trigger6 = movecontact = 1
trigger7 = stateno = 201
trigger7 = movecontact = 1
trigger8 = stateno = 202
trigger8 = movecontact = 1
[State -1]
type = ChangeState
value = 1850
triggerall = command = "c_e_alto"
trigger1 = statetype = A
trigger1 = ctrl = 1
trigger2 = stateno = 1500
[State -1]
type = ChangeState
value = 336
triggerall = command = "teleporte"
trigger1 = statetype != A
trigger1 = ctrl = 1
trigger2 = stateno = 1510
trigger3 = stateno = 1500
[State -1]
type = ChangeState
value = 2991
triggerall = command = "cutu_a"
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 1500
trigger3 = stateno = 200
trigger3 = movecontact = 1
trigger4 = stateno = 210
trigger4 = movecontact = 1
trigger5 = stateno = 211
trigger5 = movecontact = 1
trigger6 = stateno = 201
trigger6 = movecontact = 1
trigger7 = stateno = 202
trigger7 = movecontact = 1
[State -1]
type = ChangeState
value = 2992
triggerall = command = "joel_a"
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 1500
trigger3 = stateno = 200
trigger3 = movecontact = 1
trigger4 = stateno = 210
trigger4 = movecontact = 1
trigger5 = stateno = 211
trigger5 = movecontact = 1
trigger6 = stateno = 201
trigger6 = movecontact = 1
trigger7 = stateno = 202
trigger7 = movecontact = 1
[State -1]
type = ChangeState
value = 350
triggerall = command = "jog_chut"
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 1500
trigger3 = stateno = 1500
trigger4 = stateno = 200
trigger4 = movecontact = 1
trigger5 = stateno = 210
trigger5 = movecontact = 1
trigger6 = stateno = 211
trigger6 = movecontact = 1
trigger7 = stateno = 201
trigger7 = movecontact = 1
trigger8 = stateno = 202
trigger8 = movecontact = 1
[State -1]
type = ChangeState
value = 360
triggerall = command = "cut_soco"
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 1500
trigger3 = stateno = 1500
trigger4 = stateno = 200
trigger4 = movecontact = 1
trigger5 = stateno = 210
trigger5 = movecontact = 1
trigger6 = stateno = 211
trigger6 = movecontact = 1
trigger7 = stateno = 201
trigger7 = movecontact = 1
trigger8 = stateno = 202
trigger8 = movecontact = 1
[State -1]
type = ChangeState
value = 1510
trigger1 = statetype != A
trigger1 = ctrl = 1
trigger1 = command = "fly"
[State -1]
type = ChangeState
value = 1500
trigger1 = statetype = A
trigger1 = ctrl = 1
trigger1 = Pos Y < 0
trigger1 = command = "fly_air"
[State -1]
type = ChangeState
triggerall = var (5) = 0
value = 730
trigger1 = statetype = S
triggerall = Power < 4000
trigger1 = ctrl = 1
trigger1 = command = "holda"
trigger1 = command = "holdb"
trigger2 = stateno = 1500
trigger2 = command = "holda"
trigger2 = command = "holdb"
[State -1]
type = ChangeState
triggerall = var (5) = 1
value = 730989
trigger1 = statetype = S
triggerall = Power < 4000
trigger1 = ctrl = 1
trigger1 = command = "holda"
trigger1 = command = "holdb"
trigger2 = stateno = 1500
trigger2 = command = "holda"
trigger2 = command = "holdb"
[State -1]
type = ChangeState
value = 800
trigger1 = command = "balao"
trigger1 = statetype = S
trigger1 = stateno != 100
trigger1 = p2movetype != H
trigger1 = p2statetype != A
trigger1 = ctrl = 1
trigger1 = P2bodydist X <= 10
[State -1]
type = ChangeState
value = 700
triggerall = command = "ret_mag"
trigger1 = statetype != A
trigger1 = ctrl = 1
trigger2 = stateno = 1500
[State -1]
type = ChangeState
value = 100
trigger1 = command = "FF"
trigger1 = statetype = S
trigger1 = ctrl = 1
[State -1]
type = ChangeState
value = 105
trigger1 = command = "BB"
trigger1 = statetype = S
trigger1 = ctrl = 1
[State -1]
type = null; ChangeState
value = 900
triggerall = statetype = S
triggerall = ctrl = 1
triggerall = p2bodydist X < 5 ;Near P2
trigger1 = command = "fwd_b";p2 stand
trigger1 = stateno != 100    ;Not running
trigger1 = p2statetype = S
trigger1 = p2movetype != H
trigger2 = command = "fwd_b";p2 crouch
trigger2 = stateno != 100    ;Not running
trigger2 = p2statetype = C
trigger2 = p2movetype != H
trigger3 = command = "back_b";p2 stand
trigger3 = p2statetype = S
trigger3 = p2movetype != H
trigger4 = command = "back_b";p2 crouch
trigger4 = p2statetype = C
trigger4 = p2movetype != H
[State -1]
type = null; ChangeState
value = 950
triggerall = statetype = A
triggerall = ctrl = 1
triggerall = p2bodydist X < 9
triggerall = p2bodydist Y > -22
triggerall = p2bodydist Y < 22
triggerall = p2statetype = A
triggerall = p2movetype != H
trigger1 = command = "fwd_b"
trigger2 = command = "back_b"
trigger2 = stateno = 1500
trigger3 = stateno = 200
trigger4 = stateno = 201
trigger5 = stateno = 202
trigger6 = stateno = 210
trigger7 = stateno = 211
trigger8 = stateno = 212
[State -1]
type = ChangeState
value = 200
triggerall = command = "a"
triggerall = command != "holddown"
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 1500
trigger3 = stateno = 200
trigger3 = movecontact = 1
trigger4 = stateno = 201
trigger4 = movecontact = 1
trigger5 = stateno = 202
trigger5 = movecontact = 1
trigger6 = stateno = 210
trigger6 = movecontact = 1
trigger7 = stateno = 211
trigger7 = movecontact = 1
trigger8 = stateno = 212
trigger8 = movecontact = 1
[State -1]
type = ChangeState
value = 201
triggerall = command = "b"
triggerall = command != "holddown"
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 1500
trigger3 = stateno = 200
trigger3 = movecontact = 1
trigger4 = stateno = 201
trigger4 = movecontact = 1
trigger5 = stateno = 202
trigger5 = movecontact = 1
trigger6 = stateno = 210
trigger6 = movecontact = 1
trigger7 = stateno = 211
trigger7 = movecontact = 1
trigger8 = stateno = 212
trigger8 = movecontact = 1
[State -1]
type = ChangeState
value = 202
triggerall = command = "c"
triggerall = command != "holddown"
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 1500
trigger3 = stateno = 200
trigger3 = movecontact = 1
trigger4 = stateno = 201
trigger4 = movecontact = 1
trigger5 = stateno = 202
trigger5 = movecontact = 1
trigger6 = stateno = 210
trigger6 = movecontact = 1
trigger7 = stateno = 211
trigger7 = movecontact = 1
trigger8 = stateno = 212
trigger8 = movecontact = 1
[State -1]
type = ChangeState
value = 210
triggerall = command = "y"
triggerall = command != "holddown"
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 1500
trigger3 = stateno = 200
trigger3 = movecontact = 1
trigger4 = stateno = 201
trigger4 = movecontact = 1
trigger5 = stateno = 202
trigger5 = movecontact = 1
trigger6 = stateno = 210
trigger6 = movecontact = 1
trigger7 = stateno = 211
trigger7 = movecontact = 1
trigger8 = stateno = 212
trigger8 = movecontact = 1

[State -1]
type = ChangeState
value = 211
triggerall = command = "x"
triggerall = command != "holddown"
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 1500
trigger3 = stateno = 200
trigger3 = movecontact = 1
trigger4 = stateno = 201
trigger4 = movecontact = 1
trigger5 = stateno = 202
trigger5 = movecontact = 1
trigger6 = stateno = 210
trigger6 = movecontact = 1
trigger7 = stateno = 211
trigger7 = movecontact = 1
trigger8 = stateno = 212
trigger8 = movecontact = 1

;---------------------------------------------------------------------------
;Stand_z
[State -1]
type = ChangeState
value = 212
triggerall = command = "z"
triggerall = command != "holddown"
trigger1 = statetype = S
trigger1 = ctrl = 1
trigger2 = stateno = 1500
trigger3 = stateno = 200
trigger4 = stateno = 201
trigger5 = stateno = 202
trigger6 = stateno = 210
trigger7 = stateno = 211
trigger8 = stateno = 212

;---------------------------------------------------------------------------
;Crouch_a
[State -1]
type = ChangeState
value = 400
triggerall = command = "a"
triggerall = command = "holddown"
trigger1 = statetype = C
trigger1 = ctrl = 1
trigger3 = stateno = 230
trigger3 = movecontact = 1
trigger4 = stateno = 220
trigger4 = movecontact = 1
trigger5 = stateno = 210
trigger5 = movecontact = 1
trigger6 = stateno = 250
trigger6 = movecontact = 1
trigger7 = stateno = 200
trigger7 = movecontact = 1
trigger8 = stateno = 240
trigger8 = movecontact = 1
trigger9 = stateno = 420
trigger9 = movecontact = 1
trigger10 = stateno = 600
trigger10 = movecontact = 1
trigger11 = stateno = 610
trigger11 = movecontact = 1

;---------------------------------------------------------------------------
;Crouch_b
[State -1]
type = ChangeState
value = 400
triggerall = command = "b"
triggerall = command = "holddown"
trigger1 = statetype = C
trigger1 = ctrl = 1
trigger3 = stateno = 230
trigger3 = movecontact = 1
trigger4 = stateno = 220
trigger4 = movecontact = 1
trigger5 = stateno = 210
trigger5 = movecontact = 1
trigger6 = stateno = 250
trigger6 = movecontact = 1
trigger7 = stateno = 200
trigger7 = movecontact = 1
trigger8 = stateno = 400
trigger8 = movecontact = 1
trigger9 = stateno = 240
trigger9 = movecontact = 1
trigger10 = stateno = 600
trigger10 = movecontact = 1
trigger11 = stateno = 610
trigger11 = movecontact = 1

;---------------------------------------------------------------------------
;Crouch_c
[State -1]
type = ChangeState
value = 400
triggerall = command = "c"
triggerall = command = "holddown"
trigger1 = statetype = C
trigger1 = ctrl = 1

;---------------------------------------------------------------------------
;Crouch_x
[State -1]
type = ChangeState
value = 410
triggerall = command = "x"
triggerall = command = "holddown"
trigger1 = statetype = C
trigger1 = ctrl = 1
trigger3 = stateno = 230
trigger3 = movecontact = 1
trigger4 = stateno = 220
trigger4 = movecontact = 1
trigger5 = stateno = 210
trigger5 = movecontact = 1
trigger6 = stateno = 250
trigger6 = movecontact = 1
trigger7 = stateno = 200
trigger7 = movecontact = 1
trigger8 = stateno = 400
trigger8 = movecontact = 1
trigger9 = stateno = 420
trigger9 = movecontact = 1
trigger10 = stateno = 600
trigger10 = movecontact = 1
trigger11 = stateno = 610
trigger11 = movecontact = 1
;---------------------------------------------------------------------------
;Crouch_y
[State -1]
type = ChangeState
value = 410
triggerall = command = "y"
triggerall = command = "holddown"
trigger1 = statetype = C
trigger1 = ctrl = 1
trigger3 = stateno = 230
trigger3 = movecontact = 1
trigger4 = stateno = 220
trigger4 = movecontact = 1
trigger5 = stateno = 210
trigger5 = movecontact = 1
trigger6 = stateno = 250
trigger6 = movecontact = 1
trigger7 = stateno = 200
trigger7 = movecontact = 1
trigger8 = stateno = 400
trigger8 = movecontact = 1
trigger9 = stateno = 420
trigger9 = movecontact = 1
trigger10 = stateno = 600
trigger10 = movecontact = 1
trigger11 = stateno = 610
trigger11 = movecontact = 1
;---------------------------------------------------------------------------
;Crouch_z
[State -1]
type = ChangeState
value = 410
triggerall = command = "z"
triggerall = command = "holddown"
trigger1 = statetype = C
trigger1 = ctrl = 1
trigger3 = stateno = 230
trigger3 = movecontact = 1
trigger4 = stateno = 220
trigger4 = movecontact = 1
trigger5 = stateno = 210
trigger5 = movecontact = 1
trigger6 = stateno = 250
trigger6 = movecontact = 1
trigger7 = stateno = 200
trigger7 = movecontact = 1
trigger8 = stateno = 400
trigger8 = movecontact = 1
trigger9 = stateno = 420
trigger9 = movecontact = 1
trigger10 = stateno = 600
trigger10 = movecontact = 1
trigger11 = stateno = 610
trigger11 = movecontact = 1


;Jump_a
[State -1]
type = ChangeState
value = 602
trigger1 = command = "a"
trigger1 = statetype = A
trigger1 = ctrl = 1
;---------------------------------------------------------------------------
;Jump_b
[State -1]
type = ChangeState
value = 602
trigger1 = command = "b"
trigger1 = statetype = A
trigger1 = ctrl = 1
;---------------------------------------------------------------------------
;Jump_c
[State -1]
type = ChangeState
value = 602
trigger1 = command = "c"
trigger1 = statetype = A
trigger1 = ctrl = 1
;---------------------------------------------------------------------------
;Jump_x
[State -1]
type = ChangeState
value = 610
trigger1 = command = "x"
trigger1 = statetype = A
trigger1 = ctrl = 1
;---------------------------------------------------------------------------
;Jump_y
[State -1]
type = ChangeState
value = 610
trigger1 = command = "y"
trigger1 = statetype = A
trigger1 = ctrl = 1
;---------------------------------------------------------------------------
;Jump_z
[State -1]
type = ChangeState
value = 610
trigger1 = command = "z"
trigger1 = statetype = A
trigger1 = ctrl = 1

;provoc
[State -1]
type = ChangeState
value = 195
trigger1 = command = "s"
trigger1 = statetype = S
trigger1 = stateno != 312
trigger1 = ctrl = 1

;---------------------------------------------------------------------------
